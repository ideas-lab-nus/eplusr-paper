...
MLE+ {[}\protect\hyperlink{ref-Bernal2012}{28}{]} (ref:epxl) {[}\protect\hyperlink{ref-Schild2020}{30}{]} (ref:genopt) {[}\protect\hyperlink{ref-Wetter2001}{24}{]} & EnergyPlus &  & \cmark & \cmark & \cmark & Matlab &  & \cmark &  & \\
EpXL (ref:epxl) & EnergyPlus &  & \cmark & \cmark & \cmark & VBA &  & \cmark &  & \\
eppy {[}\protect\hyperlink{ref-Philip2020}{27}{]} & EnergyPlus & \cmark & \cmark &  & \cmark & Python &  &  &  & \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\scriptsize
...
suite of programming features and libraries {[}\protect\hyperlink{ref-Roth2018}{34}{]}. Among the tools,
EnergyPlus is the most used simulation engine. This may be due to its advantage
of free, open-source and cross-platform characteristics.
...
\begin{figure}[!htb]
\includegraphics[width=0.5\linewidth]{/mnt/c/Users/hongy/Dropbox/github_repo/eplusr-paper/figures/eui} \caption{Annual electricity consumption breakdown in $\mathrm{kWh}/(\mathrm{m}^2 \mathrm{a})$}\label{fig:eui}
\end{figure}
...
\begin{figure}[!htb]
\includegraphics[width=0.5\linewidth]{/mnt/c/Users/hongy/Dropbox/github_repo/eplusr-paper/figures/mtr_temp} \caption{Monthly heating and cooling demand profile}\label{fig:aircon-out}
\end{figure}
...
\CommentTok{# extract energy consumption from Output Meter}
...
\CommentTok{# extract outdoor air dry-bulb temperature from Output Variable}
...
\StringTok{    }\KeywordTok{scale_y_continuous}\NormalTok{(}\StringTok{"Energy consumption [kWh/m2]"}\NormalTok{,}
...
\NormalTok{p <-}\StringTok{ }\NormalTok{rstan}\OperatorTok{::}\KeywordTok{traceplot}\NormalTok{(}\KeywordTok{get_priv_env}\NormalTok{(bc)}\OperatorTok{$}\NormalTok{m_log}\OperatorTok{$}\NormalTok{stan}\OperatorTok{$}\NormalTok{fit, }\DataTypeTok{pars =} \KeywordTok{c}\NormalTok{(}\StringTok{"tf"}\NormalTok{, }\StringTok{"rho_eta"}\NormalTok{, }\StringTok{"sigma_e"}\NormalTok{))}

\KeywordTok{class}\NormalTok{(p)}

\KeywordTok{ggsave}\NormalTok{(}\StringTok{"/mnt/c/Users/hongy/Desktop/trace.png"}\NormalTok{, p, }\DataTypeTok{height =} \DecValTok{3}\NormalTok{, }\DataTypeTok{width =} \DecValTok{5}\NormalTok{)}

\NormalTok{rstan}\OperatorTok{::}\KeywordTok{stan_hist}\NormalTok{(}\KeywordTok{get_priv_env}\NormalTok{(bc)}\OperatorTok{$}\NormalTok{m_log}\OperatorTok{$}\NormalTok{stan}\OperatorTok{$}\NormalTok{fit)}
