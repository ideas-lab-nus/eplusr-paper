...
simulation tools developed, as shown in Table \@ref(tab:pat-sum). Some tools
...
compare <- data.table::fread(here::here("data-raw/compare.csv"),
    colClasses = c(`Calibration support` = "character")
)
data.table::setnames(compare,
    c("Optimization support", "Post-processing capabilities"),
    c(paste0("Optimization support", kableExtra::footnote_marker_number(1, "latex")),
      paste0("Post-processing capabilities", kableExtra::footnote_marker_number(2, "latex"))
    )
)
...
compare %>%
    ...
    kableExtra::kable_styling(full_width = TRUE, font_size = 7, latex_options = "hold_position") %>%
    kableExtra::column_spec(1, "2.8cm") %>%
    kableExtra::column_spec(2, "1.3cm") %>%
    kableExtra::column_spec(3, "0.4cm") %>%
    kableExtra::column_spec(4, "1.3cm") %>%
    kableExtra::column_spec(5, "1.1cm") %>%
    kableExtra::column_spec(6, "1.3cm") %>%
    kableExtra::footnote(
        number_title = "Note:",
        number = c(
            "Only built-in features are considered",
...
written in Ruby programming language to manipulate OpenStudio models can be used to compare
...
that enables import/export of IDF data files, parametric analysis and
...
mathematical optimization library. The tools mentioned above may have
...
2. Hard to setup the computational environment which covers the whole BES toolchain.
