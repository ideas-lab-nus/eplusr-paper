...
is being transformed [@Miller2013; @Attia2013a].
...
2. A parametric manager for conducting flexible and extensible parametric
...
## Parametric manager {#sec:eplusr-parametric}
...
modify an `Idf` object, including the parametric model generation process using
the `apply_measure()` method as well.
It can also be manually executed using the `validate()` method in the `Idf` class.
...
The simulation manager has an `errors()` method to parse ERR for both single
...
```{r test-coverage, eval = TRUE, include = TRUE, echo = FALSE}
coverage <- data.table::fread(here::here("data/coverage.csv"))

coverage %>%
    knitr::kable(booktabs = TRUE, format = "latex", linesep = "",
        caption = "Summary of tests and code coverage of the eplusr package") %>%
    kableExtra::kable_styling(font_size = 7, latex_options = "hold_position") %>%
    kableExtra::column_spec(1, "2.8cm") %>%
    kableExtra::column_spec(2, "2.8cm") %>%
    kableExtra::column_spec(3, "1.5cm") %>%
    kableExtra::column_spec(4, "2.0cm") %>%
    kableExtra::column_spec(5, "2.0cm")
```
...
`tabular_data()` and `report_data()` provided by the tidy data
...
```{r eui, echo = FALSE, eval = TRUE, fig.cap = "Monthly electricity consumption breakdown in $\\mathrm{kWh}/\\mathrm{m}^2$", out.width = "50%"}
...
```{r signature, echo = FALSE, eval = TRUE, fig.cap = "Energy signature diagram of outdoor temperature against electricity consumption", out.width = "50%"}
knitr::include_graphics(here::here("figures/mtr_temp.png"))
```
...
[@Ball2020open; @Karaguzel2019Open].
...
ggsave(here::here("figures/eui.png"), p_eui, width = 6, height = 3.5, dpi = 600)

ggsave(here::here("figures/mtr_temp.png"), p_mtr_temp, width = 6, height = 3.5, dpi = 600)
```
