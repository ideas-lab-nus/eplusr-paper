...
reproducible issue in BES are mainly caused by the absence of (1) an integrated
...
Parametric Analysis Tools (PAT) is a GUI application that is part of OpenStudio
...
Modelkit [@BigLadderSoftware2020] automates generation and management of
...
<!-- transition -->
GenOpt is a generic optimization programme that can be used with EnergyPlus
...
[@Baker2016a].
Currently, improving computational reproducibility has become an importance
...
errors. Currently, there is no widely adopted solution that is able to
integrate all processes into one single platform.
Currently there is no existing framework to seamlessly integrate essential
scientific processes of BES projects together, including raw design data
importing and tidying, parametric input file generation and modification,
simulation result collection, processing and analyses, and finally,
...
In order to achieve a seamless integration between BES and data-driven
...
3. Computation environment based on Docker containerisation [@Merkel2014] for
...
containerisation^[Docker Hub Link:
...
```{r architecture, echo = FALSE, eval = TRUE, fig.cap = "An architecture overview of the proposed framework which includes three main components: (1) Tidy data interface, (2) Parametric Prototype and (3) Computation environment for reproducible BES using Docker containerisation", out.width = "40%"}
...
A number of values are concatenated in column headers, including variable `Key
...
fact is expressed in only one place, similar with the Tidy principle.
...
meters with certain key identifiers, at specified time, simulation run periods,
reporting frequencies and units.
...
However, the time components in the SQL outputs fail to assemble a complete
time-series data, due to missing a year specification^[A `Year` field was added in recent version
of EnergyPlus. But old versions of EnergyPlus are still widely used.], making it
...
for each run period based on the five time columns and compose a complete
...
The Docker containerisation for BES
...
different audience and use purposes. The docker approach is
